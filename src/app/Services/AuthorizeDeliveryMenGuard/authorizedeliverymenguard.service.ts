...
export class AuthorizedeliverymenguardService implements CanActivate {
  ...
  canActivate() {
    if (localStorage.getItem("AuthAuthorities").includes(null) || localStorage.getItem("AuthAuthorities").includes("ADMINISTRATOR")
      || localStorage.getItem("AuthAuthorities").includes("CLIENT") || localStorage.getItem("AuthAuthorities").includes("DEPARTMENTMANAGER")) {
